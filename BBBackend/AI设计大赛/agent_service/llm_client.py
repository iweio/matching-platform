...
class LLMClient:
    ...
    def _simulate(cls, system_prompt: str, user_prompt: str, profile: dict | None = None) -> str:
        ...
        # ── Opening ──
        if "第一次" in user_prompt or "打招呼" in user_prompt:
            openings = {
                "活泼": [
        ...
        context_lines = []
        if "最近的对话记录" in user_prompt:
            ctx_start = user_prompt.find("最近的对话记录")
            ctx_text = user_prompt[ctx_start:]
            for line in ctx_text.split("\n"):
                if line.startswith("对方: ") or line.startswith("你: "):
        ...
        def _gen_hobby_response():
            if "活泼" in character or "外向" in speak_style:
                return rng.choice([
                    "哈哈说到爱好我可就停不下来了！我喜欢户外运动，跑步、爬山、骑行都爱，周末根本闲不住。你呢，周末一般怎么过？",
                    "我特别喜欢旅行！去过不少地方，每次都有不一样的体验。你喜不喜欢旅行呀？有没有特别想去的地方？",
                ])
        ...
        def _gen_default_response():
            ...
            if other_said:
                if "?" in other_said or "？" in other_said:
                    return rng.choice([
                        f"嗯，你问得很好。我觉得这个要看具体情况，每个人的选择都不一样。你觉得呢？",
            ...
            return rng.choice([
                "嗯，我觉得两个人在一起最重要的就是互相理解和包容。很多小事其实没必要太计较。",
                "说实话，我对感情还是比较认真的。希望能遇到一个同样真诚的人。",
                "和你聊天感觉还不错，能感受到你是一个有想法的人。继续聊聊吧～",
            ])

        # Map user_prompt keywords to response generators
        if "兴趣爱好" in user_prompt:
            return _gen_hobby_response()
        if "工作" in user_prompt or "生活状态" in user_prompt:
            return _gen_work_response()
        if "感情" in user_prompt:
            return _gen_love_view_response()
        if "家庭" in user_prompt:
            return _gen_family_response()
        if "未来" in user_prompt or "规划" in user_prompt:
            return _gen_future_response()
        if "婚姻" in user_prompt:
            return _gen_marriage_response()
        if "消费" in user_prompt:
            return _gen_consumption_response()
        if "习惯" in user_prompt:
            return _gen_habit_response()
        if "总结" in user_prompt or "结束" in user_prompt:
            return _gen_summary_response()
        return _gen_default_response()
